...
Le dépôt versionne ses hooks dans `.githooks/`. Pour les activer sur un clone :

```shell
git config core.hooksPath .githooks
```

Le hook `commit-msg` retire de tout message de commit les **liens de
chat/session** (`Claude-Session: …`, `claude.ai/code/session`) qu'un outillage
d'agent peut injecter d'office — cf. `CLAUDE.md`. Un `Co-Authored-By: Claude …`
est en revanche **toléré** (attribution) et laissé tel quel. Le filet de
sécurité reste le job CI **« Attribution propre »**, qui bloque une PR même si le
hook local n'est pas activé : il vérifie les **messages de commit** (lien de
session), l'**identité** (auteur / committer — qui doivent être les vôtres, pas
ceux d'un agent) **et** le **corps de la PR** (où le pied de page
`_Generated by [Claude Code](…session…)_` est ré-injecté d'office).
